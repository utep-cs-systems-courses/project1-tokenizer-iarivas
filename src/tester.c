...
static char *the_string_length() {
    mu_assert("string_length('happy') == 5", string_length("happy") == 5);
    return 0;
}

static char *is_valid_character() {
    mu_assert("is_valid_character(' ') == 0", valid_character(' ') == 0);
    mu_assert("is_valid_character('h') == 1", valid_character('h') == 1);
    return 0;
}
...
static char *all_tests() {
    if (TEST_TOKENIZER) {
      mu_run_test(the_string_length);
      mu_run_test(is_valid_character);
